...
# Mouse interaction http://www.glowscript.org/docs/VPythonDocs/mouse.html
...
wall_1 = box(pos=vector(center.x-side/2,center.y,center.z), length=thickness, height=side, width=side, color=color.red, opacity=0.3)
wall_2 = box(pos=vector(center.x+side/2,center.y,center.z), length=thickness, height=side, width=side, color=color.red, opacity=0.3)
wall_3 = box(pos=vector(center.x,center.y,center.z+side/2), length=side, height=side, width=thickness, color=color.white, opacity=0.3)
wall_4 = box(pos=vector(center.x,center.y,center.z-side/2), length=side, height=side, width=thickness, color=color.white, opacity=0.3)

ball = sphere(pos=vector(0,1,0), color=color.red, radius=0.5)
...
while 1:
    ...
    if over_floor(ball, floor) and (ball.pos.y - ball.radius <= floor.pos.y):
        ball.velocity.y *= -1
    else:
